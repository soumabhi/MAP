...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _handleLogin() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      print('➡️ Branch ID: $branchId');
      print('➡️ Password: ${'*' * password.length}');

      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      final success = await authProvider.login(branchId, password);
      ...
      if (success) {
        ...
        if (mounted) {
          _showErrorDialog(context, authProvider.error ?? 'Login failed');
        }
      }
    } else {
  ...
  Widget _buildBranchIdField(bool isTablet) {
  return TextFormField(
    controller: _branchIdController,
    style: TextStyle(fontSize: isTablet ? 18 : 16),
    decoration: InputDecoration(
